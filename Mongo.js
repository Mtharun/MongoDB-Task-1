...
//1. to find the document
db.product.find();
//it --> //db.product.find().toArray();

//2.find Product Price Between 400 to 800
db.product.find({product_price:{$gt:400.00,$lt:800.00}})

//3.find Product Price Not Between 400 to 800
db.product.find({product_price:{$not:{$gt:400.00,$lt:800.00}}})

//4.List four Product which are greater than 500 in price
db.product.find({product_price:{$gte:500.00}}).limit(4);
//db.product.find({product_price:{$gte:500.00}});

//5.Find the productname and product material
db.product.find().toArray();
db.product.find().forEach(function (pro) {
    print("Product_name : " + pro.product_name +"  "+ ",  Product_material : "  + pro.product_material);
});
...
db.product.aggregate([
    ...
    doc.duplicate.shift();
    db.product.deleteMany({_id:{$in : doc.duplicate}})
})